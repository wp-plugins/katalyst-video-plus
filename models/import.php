...
abstract class KVP_Importer {
	...
	public function import( $source, $action = false ) {
		...
		if( isset($this->source['import']['is_importing']) && true == $this->source['import']['is_importing'] && isset($this->source['import']['action']) && in_array($this->source['import']['action'], array('import', 'audit', 'cron', 'ajax') ) && $this->source['import']['action'] !== $this->action ) {
	...
	private function import_start() {
		...
		if( isset($this->source['import']['action']) && !in_array($this->source['import']['action'], array('import', 'audit', 'cron', 'ajax') ) )
			$this->source['import']['action'] = $this->action;
		...
		if( in_array($this->source['import']['action'], array('import', 'audit', 'cron', 'ajax') ) && empty($this->source['import']['queued']) ) {
			...
			foreach( $imported_ids as $import_id ) {
				...
				if( $import_id['provider'] === $this->source['provider'] ){
					...
					if( ( 'audit' !== $this->source['import']['action'] && true == array_key_exists($import_id['ID'], $this->source['import']['queued']) ) || ( 'audit' == $this->source['import']['action'] && false === array_key_exists($import_id['ID'], $this->source['import']['queued']) ) ) {
						
						unset($this->source['import']['queued'][$import_id['ID']]);
					
					}
					
					if( 'audit' === $this->source['import']['action'] )
						$this->source['import']['queued'][$import_id['ID']]['post_id'] = $import_id['post_id'];
				}
		...
		if( in_array($this->source['import']['action'], array('import', 'audit', 'cron', 'ajax') ) && false == $this->source['import']['is_importing'] )
	...
	private function import_end() {
		...
		$this->source['import']['action']		= false;
		$this->update_source();
	...
	private function process_post() {
		...
		if( isset($this->source['import']['queued'][$this->active['ID']]['post_id']) && is_int($this->source['import']['queued'][$this->active['ID']]['post_id']) && 'audit' !== $this->source['import']['action'] )
		...
		if( 'audit' !== $this->source['import']['action'] )
			...
			$this->status( __('Updating post.', 'kvp') );
		...
		if( 'audit' === $this->source['import']['action'] )
			$post['ID'] = $this->source['import']['queued'][$this->active['ID']]['post_id'];
